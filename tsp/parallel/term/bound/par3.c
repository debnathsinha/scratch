...
int dfs(int city,int bitv, int cost)
{
  ...
  if(bitv==0)
      ...
      if(upperb>(cost+adj[city][snode]))
	{
	  ...
	  printf("Upper bound is being revised by %d to %d\n",myid,cost+adj[city][snode]);
	}
  ...
  if(myid==0)
    {
      ...
      printf("The time taken for the bfs was %lf and %d nodes were successful and dfscount:%d\n",end-start,suc,dfscount);
    }
  else
    {
      ...
      if(t[0]==-1 || t[1]==-1 || t[2]==-1)
	{
	  ...
	  */
	  ...
	  printf("Btw...%d nodes were successful and %d were not and dfs count:%d\n",suc,suc1,dfscount);
	  }
    }
  end=MPI_Wtime();
