...
/*There seems to be some problem with the queue*/
# include<stdio.h>
# include<math.h>
# include<mpi.h>
# include<stdlib.h>
...
int adj[n][n];
int cache[n][(1<<n)];
int queue[30000][3],upperb;
int snode=0,ubupdate,k=10;
int front=0,rear=0;
double suc,suc1;
int ubtag=56,rsend=36,rrecv=46;
MPI_Status stat;
MPI_Request req;
int lbcostarr[n],lbposiarr[n],inilb,k1=0,c;
...
int dfs(int city,int bitv, int cost)
{
  ...
  if((MPI_Wtime()-lastupdated)>thresh)
    {
      printf("Entering the if condition \n");
