...
/*Alternatives to broadcasting by sending to everyone*/
...
int dfs(int city,int bitv, int cost)
{
  ...
  if(bitv==0)
      ...
      if(upperb>(cost+adj[city][snode]))
	{
	  ...
	  if(MPI_Wtime()-last_updated>2.0)
	    for(i=1;i<n,i!=myid;i++)
	      {
		printf("Trying to send to %d\n",i);
		MPI_Send(&upperb,1,MPI_INT,i,ubtag,MPI_COMM_WORLD);
		last_updated=MPI_Wtime();
		printf("I %d sent to %d and shifting last+updated to %lf\n",myid,i,last_updated);
	      }
	  else
