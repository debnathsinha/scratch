...
int ubtag=56,rsend=3,rrecv=4;
MPI_Status stat;
MPI_Request req;
int lbcostarr[n],lbposiarr[n],inilb;
double lastupdated;
...
int main(int argc, char *argv[])
{
  ...
  int i,bitv,mincost,t[3],mycost,sflag=0,tupperb;
  ...
  */
  ...
  if(myid==0)
      ...
      while(t[0]!=-1 || t[1]!=-1 || t[2]!=-1)
	{
	  ...
	  printf("The mssg recv has tag:%d source:%d\n",stat.MPI_TAG,stat.MPI_SOURCE);
	  if(stat.MPI_TAG==rsend)
	    {
	      MPI_Recv(t,3,MPI_INT,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&stat);
	      printf("%d AM RECEIVING tag:%d FROM %d\n",myid,stat.MPI_TAG,stat.MPI_SOURCE);
	      mycost=0;
	      mycost=dfs(t[0],t[1],t[2]);
	      mycost=t[2]+mycost;
	      MPI_Send(&mycost,1,MPI_INT,0,rrecv,MPI_COMM_WORLD);
	      printf("Sending root with %d\n",mycost);
	    }
	  else
	    {
	      printf("%d am receiving tag:%d from %d",myid,stat.MPI_TAG,stat.MPI_SOURCE);
	      MPI_Recv(&tupperb,1,MPI_INT,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&stat);
	      if(tupperb<upperb)
		upperb=tupperb;
	      lastupdated=MPI_Wtime();
	      printf("Value that was received by %d %d and this is the actual upper bound %d!!!\n",myid,tupperb,upperb);
	    }
	}
