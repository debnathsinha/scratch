  ...
  while(size()>0)
    {
      ...
      MPI_Isend(t,3,MPI_INT,count1,0,MPI_COMM_WORLD,&request);
      nsend++;
  ...
  for(i=1;i<numprocs;i++)
    {
      ...
      MPI_Isend(arr,3,MPI_INT,i,0,MPI_COMM_WORLD,&request);
...
int main(int argc, char *argv[])
{
  ...
  double start,end,procstart,procend;
  ...
  */
  ...
  if(myid==0)
      ...
      while(t[0]!=-1 || t[1]!=-1 || t[2]!=-1)
	{
	  ...
	  procend=MPI_Wtime();
	  printf("%d took %lf time to compute\n",myid,procend-procstart);
	  MPI_Recv(t,3,MPI_INT,0,0,MPI_COMM_WORLD,&stat);
	  procend=MPI_Wtime();
	  printf("%d TOOK %lf TIME TO RECEIVE NEXT\n",myid,procend-procstart);
	}
