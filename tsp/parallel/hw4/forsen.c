...
double queue[20000][4],tempq[20000][4];
int front=0, rear=0;
int numprocs,myid,k=150;
double sum1=0.0;
double startime;
MPI_Status stat;
...
void bfs()
{
  ...
  */
  ...
  while(size()>0)
	...
	{
	  t=pop();
	  MPI_Send(t,4,MPI_DOUBLE,count1,0,MPI_COMM_WORLD);
	}
      /*Processor 0 doing its own work*/
      t=pop();
...
int main(int argc, char *argv[])
{
  ...
  if(myid==0)
  ...
  else
    {
      ...
      /*printf("%lf %lf %lf %lf : %d\n",queue[0][0],queue[0][1],queue[0][2],queue[0][3],myid);*/
      while(queue[0][1]!=-1)
	{
	  MPI_Recv(queue[0],4,MPI_DOUBLE,0,0,MPI_COMM_WORLD,&stat);
	  anssend=0.0;
