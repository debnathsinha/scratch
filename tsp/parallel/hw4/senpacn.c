...
double queue[50000][4],tempq[50000][4];
int front=0, rear=0;
int numprocs,myid,k=10;
double sum1=0.0;
double startime;
MPI_Status stat;
...
void bfs()
{
  ...
  printf("Displaying the original queue of size : %d\n",size());
  ...
  for(count1=1;count1<numprocs;count1++)
    {
      printf("\nProcessor : %d\n",count1);
      for(count2=rear,count3=0;count2<front;count2+=numprocs,count3++)
	{
      ...
      MPI_Send(tempq,(int)(4*k/numprocs),MPI_DOUBLE,count1,0,MPI_COMM_WORLD);
    }
...
int main(int argc, char *argv[])
{
  ...
  if(myid==0)
  ...
  else
    {
      ...
      MPI_Recv(queue,(int)(4*k/numprocs),MPI_DOUBLE,0,0,MPI_COMM_WORLD,&stat);
      for(count1=0;count1<k;count1++)
	{
