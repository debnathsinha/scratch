...
def readFile():
    return file("big.txt").read()

def parseWords(fslurp):
    return re.findall('[a-z]+',fslurp.lower())
...
WORDS_MODEL=train(parseWords(readFile()))

def edits(word):
    splits = [(word[:i],word[i:]) for i in range(len(word) + 1)]
    deletions = [a + b[1:] for a,b in splits if b]
    insertions = [ a + c + b for a,b in splits for c in alphabets ]
    replaces = [ a + c + b[1:] for a,b in splits if b for c in alphabets ]
    transposes = [ a + b[1] + b[0] + b[2:] for a,b in splits if len(b) >= 2]
    assert( len(deletions) == len(word) )
    assert( len(insertions) == len(alphabets) * ( len(word) + 1 ) )
    assert( len(transposes) == len(word) - 1 )
    assert( len(replaces) == len(alphabets) * len(word) )
    return set( deletions + insertions + replaces + transposes )

def edits2(word):
    return set( e2 for e1 in edits(word) for e2 in edits(e1) if e2 in WORDS_MODEL )

def analyze(word):
    edits(word)

def known(words):
    return set( w for w in words if w in WORDS_MODEL )

def correct(word):
    candidates = known([word]) or known(edits(word)) or known(edits2(word))
    return max( candidates, key = WORDS_MODEL.get )

def parseDict():
    print "Parsing dictionary"
    fslurp = readFile()
    words = parseWords(fslurp)
    train(words)

if __name__ == "__main__":
    print "Starting spell check"
    parseDict()
    assert( edits2("smoothing") == set( ['soothing', 'shooting', 'something', 'smoothing'] ) )
    assert( len(edits2("smoothing")) == 4 )
    print correct("corect")
...
def spelltest(tests, bias=None, verbose=False):
    ...
    if bias:
        for target in tests: WORDS_MODEL[target] += bias
    for target, wrongs in tests.items():
        for wrong in wrongs.split(" "):
            print "target:%s,wrong:%s" % (target, wrong)
            n+= 1
            w = correct(wrong)
            if w != target:
                print w + target
                bad += 1
                unknown += (target not in WORDS_MODEL)
                if verbose:
                    print "%r => %r (%d); expected %r (%d)" % (
                        wrong, w, WORDS_MODEL[w], target, WORDS_MODEL[target])
